        ...
        events.forEach(event => {
            ...
            switch (event.category) {
                case "note":
                    ...
                    break;
                case "activity":
                    code += ((icons[event.extra.type] ? icons[event.extra.type] : icons["Workout"]) + ' ' + event.title + ' ' + privateLabel + ' <span class="hidden-sm-up"><br/></span><span style="color:#CCC;font-size:14px"> ' + (event.extra.distance !== 0 ? (event.extra.distance / 1000).toFixed(1) + 'km | ' : '') + formatDuration(event.extra.moving_time) + (event.extra.average_speed !== 0 ? (' | ' + formatSpeed(event.extra.average_speed, event.extra.type)) : '') + '</span > <span class="hidden-sm-down" style="color:#CCC;font-size:14px">' + (event.extra.total_elevation_gain !== 0 ? ' | ' + Math.round(event.extra.total_elevation_gain) + 'm' : '') + (event.extra.average_heartrate ? ' | ' + Math.round(event.extra.average_heartrate) + 'bpm' : '') + '</span>')
                    break;
                case "fireman":
